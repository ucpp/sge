...
namespace SGE
{
    void Texture::Initialize(const std::string& texturePath, Device* device, DescriptorHeap* descriptorHeap, uint32 descriptorIndex)
    {
        ...
        {
            Verify(LoadFromDDSFile(fileName.c_str(), DDS_FLAGS_FORCE_RGB, &metadata, scratchImage));
        }
        ...
        auto scratchImageData = scratchImage.GetImage(0, 0, 0);
        textureData.pData = scratchImageData->pixels;
        textureData.RowPitch = static_cast<LONG_PTR>(scratchImageData->rowPitch);
        textureData.SlicePitch = static_cast<LONG_PTR>(scratchImageData->slicePitch);

        ID3D12GraphicsCommandList* commandList = device->GetCommandList().Get();

        UpdateSubresources(commandList, m_resource.Get(), m_resourceUpload.Get(), 0, 0, subresourceCount, &textureData);
